...
public class Lambda03 {
	public static void main(String[] args) {
		...
		list.stream().map(t -> t * t) // map() gets the element, updates it according to the right part (t*t)
										// and navigates to the next line
				.forEach(System.out::print);

		System.out.println("====");
		// Print the cube of every odd elements
		list.stream().filter(t -> t % 2 != 0).map(t -> t * t * t + " ").forEach(System.out::print);

		System.out.println("====");
		// Print the square of the elements which are divisible by 4
		list.stream().filter(t -> t % 4 == 0) // First filter than use map()
				.map(t -> t * t + " ").forEach(System.out::print);
